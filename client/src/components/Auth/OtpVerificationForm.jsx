...
const OTP = () => {
  ...
  const [email, setEmail] = useLocalStorage("email",''); // Track the email address
  ...
  return (
    <div className="w-full h-screen bg-slate-900 flex justify-center items-center">
      <div className="bg-[#c2c5ff] p-6 rounded-lg shadow-lg w-4/5 sm:w-96 md:w-1/3 lg:w-1/4 text-[#152250]">
        ...
        <div className="grid grid-cols-4 gap-2 mb-6 justify-center">
          {otp.map((digit, index) => (
            <input
              key={index}
              id={`otp-input-${index}`}
              className="w-16 h-16 text-center text-xl font-semibold rounded-md focus:ring-2 focus:ring-[#152250] focus:outline-none"
              type="text"
              inputMode="numeric"
              maxLength={1}
              value={digit}
        ...
        <button
          type="button"
          className="w-full py-2 px-4 oauthButton font-semibold rounded-xl focus:outline-none disabled:bg-gray-300"
